...
def execute_web_search(
    ...
    *,
    ...
    if not getattr(settings, "WEB_SEARCH_ENABLED", True):
        logger.info("Web search is disabled via WEB_SEARCH_ENABLED=false")
        return []
    ...
    # Redis cache check
    query_hash = hashlib.sha256(clean_query.casefold().encode("utf-8")).hexdigest()
    cache_key = f"web_search:{provider}:{limit}:{query_hash}"
